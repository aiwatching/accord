...
if grep -q "^scope:" "$FILE"; then
    ...
    case "$scope_value" in
        external|internal) ;;
        *) echo "  FAIL: Invalid scope: $scope_value (must be external or internal)"; ((ERRORS++)) ;;
    esac
fi
...
if grep -q "^status:" "$FILE"; then
    ...
    case "$status_value" in
        ...
        *) echo "  FAIL: Invalid status: $status_value"; ((ERRORS++)) ;;
    esac
fi
...
if grep -q "^priority:" "$FILE"; then
    ...
    case "$priority_value" in
        ...
        *) echo "  FAIL: Invalid priority: $priority_value"; ((ERRORS++)) ;;
    esac
fi
...
if grep -q "^type:" "$FILE"; then
    ...
    case "$type_value" in
        ...
        *) echo "  WARN: Non-standard request type: $type_value"; ((WARNINGS++)) ;;
    esac
fi
