...
This example demonstrates a realistic Accord setup for a Network Access Control (NAC) system using the **monorepo** model.
...
├── .accord/
│   └── config.yaml                         # Project-level config (monorepo, 4 teams)
│
├── contracts/                              # External contracts (OpenAPI)
...
│
├── .agent-comms/                           # Cross-team communication
...
├── device-manager/
│   ├── .accord/
│   │   ├── config.yaml                     # Service config (3 modules)
│   │   └── internal-contracts/             # Collected copies
│   │       ├── plugin.md
│   │       ├── discovery.md
│   │       └── lifecycle.md
...
│   │   │   └── lifecycle/
│   │   └── archive/
│   ├── plugin/.accord/contract.md          # Source of truth
│   ├── discovery/.accord/contract.md       # Source of truth
│   └── lifecycle/.accord/contract.md       # Source of truth
└── README.md                               # This file
```
...
2. Created `req-001-policy-by-type.md` in `.agent-comms/inbox/nac-engine/`
3. nac-engine team approved and implemented the endpoint
4. Request was moved to `.agent-comms/archive/` with status `completed`
5. The endpoint is now in `contracts/nac-engine.yaml` (stable, no annotations)
...
2. Created `req-002-rbac-permissions.md` in `.agent-comms/inbox/nac-admin/`
3. The proposed endpoint is annotated in `contracts/nac-admin.yaml` with `x-accord-status: proposed`
4. Waiting for nac-admin team to review and approve

## How to Use This Example

1. Study the directory structure to understand the Accord layout
2. Read `.accord/config.yaml` to see project configuration
3. Read the contracts in `contracts/` to see external API boundaries
4. Read the internal contracts in `device-manager/` to see module-level boundaries
